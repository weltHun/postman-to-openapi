    ...
    if (element != null) {
      ...
      if (valid) {
        ...
        if (description !== undefined && description.includes('#')) {
          const lineWithHash = description.split('\n').find(line => line.includes('#')).replace('#app-', '')
          tag = lineWithHash
          tags[tag] = lineWithHash
        }
        paths[joinedPath][method.toLowerCase()] = {
          ...
          ...parseOperationAuth(postmanJson, securitySchemes, optsAuth),
